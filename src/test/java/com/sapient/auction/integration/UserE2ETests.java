...
public class UserE2ETests extends BaseE2ETest {
     ...
     * Exptected: failed 400 Bad request, User id being user..
     */
    @Test
    public void testRegisterWithExistingUserId() {

        //create user1.
        AuctionResponse response = registerUser("treddy42@sapient.com");
        assertEquals(response.getStatusCode(), Response.Status.CREATED.getStatusCode());
        assertEquals(response.getMessage(), "User registration successful.");

        //create user2.
        response = registerUser("treddy42@sapient.com");
    ...
    public void testLogin() {
        //create user1.
        AuctionResponse response = registerUser("treddy43@sapient.com");
        assertEquals(response.getStatusCode(), Response.Status.CREATED.getStatusCode());
        assertEquals(response.getMessage(), "User registration successful.");

        //Login.
        HttpEntity httpEntity = new HttpEntity(user("treddy43@sapient.com"), httpHeaders);
     ...
     * Expected: failed with 404-NotFound. User Not Found.
     */
    @Test
    public void testLoginFailed() {
        UserVO user = user("treddy44@sapient.com");
